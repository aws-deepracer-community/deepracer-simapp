...
from deepracer_env.log_handler import exception_handler
from deepracer_env.log_handler.constants import (SIMAPP_TRAINING_WORKER_EXCEPTION,
...
@pytest.mark.parametrize("message, exceptionType, eventType, errorCode",
                         [("Sample Training Worker Exception", SIMAPP_TRAINING_WORKER_EXCEPTION,
                          ...
                          ("Sample Training Worker Exception", SIMAPP_TRAINING_WORKER_EXCEPTION,
                           SIMAPP_EVENT_USER_ERROR, SIMAPP_EVENT_ERROR_CODE_400)])
def test_log_and_exit(message, exceptionType, eventType, errorCode):
    """The test function checks if the log_and_exit() function from exception_handler.py
    once called, is able to log the appropriate error message and abort the entire program.
