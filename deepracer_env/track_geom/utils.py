...
import numpy as np
import math
from deepracer_env.track_geom.constants import START_POS_OFFSET, HIDE_POS_OFFSET, HIDE_POS_DELTA
...
def get_start_positions(race_car_num, start_pos_offset=START_POS_OFFSET):
    """Get start positions based on number of race cars and start position offset.

    Args:
        race_car_num (int): The number of race cars in current environment.
        start_pos_offset (float): The start position offset for agent.
