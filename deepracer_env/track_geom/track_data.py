...
from deepracer_env.agent_ctrl.constants import RewardParam
from deepracer_env.cameras.frustum_manager import FrustumManager
from deepracer_env.track_geom.constants import TrackNearPnts, TrackNearDist, ParkLocation
from deepracer_env.track_geom.utils import euler_to_quaternion, apply_orientation, find_prev_next, quaternion_to_euler
    ...
    def get_collided_object_name(self, racecar_name):
        ...
        try:
            with self.noncollidable_object_lock:
                ...
                object_bounding_points = TrackData.get_object_bounding_rect(racecar_pose, racecar_dims)
            for object_name in self.object_poses.keys():
                if object_name != racecar_name and object_name not in noncollidable_objects:
                    ...
                    if any([object_boundary.contains(Point(p)) for p in object_bounding_points]):
                        return object_name
            return ''
        except Exception as ex:
