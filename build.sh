...
for a in $ARCH; do   
    ...
    if [ "$arch_p1" == "cpu" ]; then
        ...
        if [ "$arch_p2" == "gl" ]; then
            ...
            docker build . -t $PREFIX/deepracer-robomaker:${VERSION}-cpu-gl-${arch_tf} -f docker/Dockerfile.cpu-gl --build-arg TENSORFLOW_VER=$tf
        else
            arch_tf=$arch_p2
            tf=$(eval echo $TF_PATH)
            docker build . -t $PREFIX/deepracer-robomaker:${VERSION}-cpu-${arch_p2} -f docker/Dockerfile.cpu --build-arg TENSORFLOW_VER=$tf
        fi 
    elif [ "$arch_p1" == "gpu" ]; then
        docker build . -t $PREFIX/deepracer-robomaker:${VERSION}-${a} -f docker/Dockerfile.${a}
    fi
done
