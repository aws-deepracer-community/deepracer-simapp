...
/usr/bin/python3.8 /opt/ml/code/scripts/cloudwatch_uploader.py --cw_log_group_name ${SIMULATION_LOG_GROUP} --cw_log_stream_name ${JOB_NAME} --log_symlink_file_path /opt/ml/simapp.log &
/usr/bin/python3.8 /opt/ml/code/scripts/cloudwatch_uploader.py --cw_log_group_name ${TRAINING_LOG_GROUP} --cw_log_stream_name ${JOB_NAME} --log_symlink_file_path /opt/ml/training.log &
...
python3.8 /opt/amazon/install/sagemaker_rl_agent/lib/python3.8/site-packages/markov/training_worker.py $@ > /opt/ml/training.log 2>&1 &
conda deactivate
