...
home = expanduser("~")
"""
this file is only created when simapp_exception happens. We dont want
to mark SM job as failed in this case
"""
...
def get_env_variables(user_args_dict, is_sageonly=False):
    ...
    # For virtual event jobs MODEL_METADATA_FILE_S3_KEY is not needed and thus not passed
    # So to avoid key not present error, using .get to set it to blank value instead by default
    env["MODEL_METADATA_FILE_S3_KEY"] = user_args_dict.get("--model_metadata_s3_key", "") 
    if is_sageonly:
        ...
        # .get method is safe check when retrieving values.
        ...
        if env["SIMULATION_LAUNCH_FILE"] == "distributed_training.launch":
            ...
            env["CRASH_STATUS_FILE_NAME"] = user_args_dict["--crash_status_file_name"]
        else:
...
def run_command(cmd, env_args):
    ...
    # throw error execpt for cases when simapp_exception happens
    if retval != 0 and not os.path.isfile(EXCEPTION_HANDLER_SYNC_FILE):
