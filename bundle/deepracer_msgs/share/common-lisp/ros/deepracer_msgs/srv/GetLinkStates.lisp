...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<GetLinkStates-request>)))
  ...
  (cl:format cl:nil "string[] link_names          # name of link~%                             # link names are prefixed by model name, e.g. pr2::base_link~%string[] reference_frames    # reference frame of returned information, must be a valid link~%                             # if empty, use inertial (gazebo world) frame~%                             # reference_frame names are prefixed by model name, e.g. pr2::base_link~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'GetLinkStates-request)))
  "Returns full string definition for message of type 'GetLinkStates-request"
  (cl:format cl:nil "string[] link_names          # name of link~%                             # link names are prefixed by model name, e.g. pr2::base_link~%string[] reference_frames    # reference frame of returned information, must be a valid link~%                             # if empty, use inertial (gazebo world) frame~%                             # reference_frame names are prefixed by model name, e.g. pr2::base_link~%~%~%"))
