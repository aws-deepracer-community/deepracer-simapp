...
try:
    ...
    failed_imports.append("ALE (Arcade Learning Environment)")
...
class MaxOverFramesAndFrameskipEnvWrapper(gym.Wrapper):
    ...
    def reset(self):
        # Gymnasium compatibility: reset() returns (observation, info)
        reset_result = self.env.reset()
            
        if isinstance(reset_result, tuple) and len(reset_result) == 2:
            # New Gymnasium format: (observation, info)
    ...
    def step(self, action):
        ...
        for i in range(self.frameskip):
            # Gymnasium compatibility: handle both 4-value and 5-value returns
            step_result = self.env.step(action)
            if len(step_result) == 5:
                # New Gymnasium format: (observation, reward, terminated, truncated, info)
                observation, reward, terminated, truncated, info = step_result
                done = terminated or truncated
            else:
                # Old gym format: (observation, reward, done, info)
...
class GymEnvironment(Environment):
    def __init__(self,
        ...
        # hide warnings - using gymnasium API
        ...
        if self.seed is not None:
            # Gymnasium compatibility: use reset with seed parameter
            try:
                # Try new Gymnasium seeding method
                ...
                # Fallback to old gym seeding method if available
                try:
                    self.env.seed(self.seed)
                except AttributeError:
                    pass  # Environment doesn't support seeding
            
            # Always seed numpy and random for reproducibility
    ...
    def _take_action(self, action):
        ...
        if len(step_result) == 5:
            # New Gymnasium format: (observation, reward, terminated, truncated, info)
            self.state, self.reward, terminated, truncated, self.info = step_result
            self.done = terminated or truncated
        else:
            # Old gym format: (observation, reward, done, info)
            self.state, self.reward, self.done, self.info = step_result
    ...
    def _restart_environment_episode(self, force_environment_reset=False):
        ...
        if (self.is_atari_env and self.env.unwrapped.ale.lives() > 0) \
            ...
            # Gymnasium compatibility: reset() returns (observation, info)
            # Try to pass seed to reset for proper Gymnasium seeding
            if self.seed is not None:
                try:
                    reset_result = self.env.reset(seed=self.seed)
                except TypeError:
                    # Environment doesn't support seed parameter in reset
                    reset_result = self.env.reset()
            else:
                reset_result = self.env.reset()
