...
def change_x_axis(attr, old, new):
    global x_axis
    show_spinner("Updating the X axis...")
    x_axis[0] = x_axis_options[new]
    plot.xaxis.axis_label = x_axis_labels[new]
...
def toggle_group_property(attr, old, new):
...
x_axis_selector.on_change('active', change_x_axis)
...
averaging_slider.js_on_change('value', CustomJS(args=dict(source=averaging_slider_dummy_source), code="""
    source.data = { value: [cb_obj.value] }
"""))

# group properties checkbox
group_cb = CheckboxGroup(labels=["Show statistics bands", "Ungroup signals"], active=[])
group_cb.on_change('active', toggle_group_property)
...
# Configure plot toolbar directly instead of using deprecated ToolbarBox
