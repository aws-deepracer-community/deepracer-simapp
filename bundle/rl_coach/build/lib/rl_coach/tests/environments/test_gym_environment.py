...
@pytest.fixture()
def continuous_env():
    # create a pendulum gym environment
    env = GymEnvironment(level='Pendulum-v1',
                         seed=1,
                         frame_skip=1,
                         visualization_parameters=VisualizationParameters())
    env.reset_internal_state(True)
...
def test_gym_continuous_environment(continuous_env):
    ...
    # make sure that the seed is working properly
    assert np.sum(continuous_env.last_env_response.next_state['observation']) == 1.972379207611084
