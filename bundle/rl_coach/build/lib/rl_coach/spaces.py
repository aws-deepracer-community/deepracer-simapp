...
class Space(object):
    ...
    def contains(self, val: Union[int, float, np.ndarray]) -> bool:
        ...
        if type(val) == np.ndarray:
            # Handle shape comparison safely to avoid broadcasting errors
            try:
                if not np.all(val.shape == self.shape):
                    return False
            except ValueError:
                # Shapes can't be compared due to different dimensions
                return False
...
class DiscreteActionSpace(ActionSpace):
    ...
    @property
    def actions(self) -> List[ActionType]:
        # Handle both scalar and array formats for gymnasium compatibility
        try:
            # Try array indexing first
            high_val = self.high[0]
        except (IndexError, TypeError):
            # If indexing fails, use the scalar value directly
            high_val = self.high
        return list(range(0, int(high_val) + 1))
    ...
    def sample_with_info(self) -> ActionInfo:
        ...
        try:
            ...
            # If indexing fails, use the scalar values directly
            high_val = self.high
            low_val = self.low
        
        return ActionInfo(self.sample(),
                          all_action_probabilities=np.full(len(self.actions), 1. / (high_val - low_val + 1)))
