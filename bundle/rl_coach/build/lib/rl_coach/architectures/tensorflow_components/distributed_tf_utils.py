...
def create_cluster_spec(parameters_server: str, workers: str) -> tf.compat.v1.train.ClusterSpec:
    """
    ...
    cluster_spec = tf.compat.v1.train.ClusterSpec({"ps": ps_hosts, "worker": worker_hosts})

    return cluster_spec


def create_and_start_parameters_server(cluster_spec: tf.compat.v1.train.ClusterSpec, config: tf.compat.v1.ConfigProto=None) -> None:
    """
...
def create_worker_server_and_device(cluster_spec: tf.compat.v1.train.ClusterSpec, task_index: int,
