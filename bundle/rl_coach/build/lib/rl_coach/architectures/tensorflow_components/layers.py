...
def batchnorm_activation_dropout(input_layer, batchnorm, activation_function, dropout_rate, is_training, name):
    ...
    # batchnorm
    if batchnorm:
        bn_layer = tf.keras.layers.BatchNormalization(name="{}_batchnorm".format(name))
        layers.append(bn_layer(layers[-1], training=is_training))
    ...
    # dropout
    if dropout_rate > 0:
        dropout_layer = tf.keras.layers.Dropout(rate=dropout_rate, name="{}_dropout".format(name))
        layers.append(dropout_layer(layers[-1], training=is_training))
...
class Dense(layers.Dense):
    ...
    def __call__(self, input_layer, name: str=None, kernel_initializer=None, bias_initializer=None,
        ...
        # Use modern Keras API instead of deprecated tf.compat.v1.layers
