...
class S3Writer(object):
    ...
    def upload_to_s3(self):
        ...
        try:
            ...
            upload_result = multiprocess_pool.starmap_async(self._multiprocess_upload_s3,
                                      [(job.s3_bucket, job.s3_prefix, job.aws_region, job.local_file)
                                       for job in self.job_info])
            upload_result.wait(10)
            if upload_result.ready() is not True:
                print("ERROR: Upload was not completed")
