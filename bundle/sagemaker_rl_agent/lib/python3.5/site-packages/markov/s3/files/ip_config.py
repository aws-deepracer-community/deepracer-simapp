...
class IpConfig():
    ...
    def persist(self, s3_kms_extra_args):
        ...
        LOG.info("[s3] Successfully upload ip address to \
        ...
        self._s3_client.upload_fileobj(bucket=self._bucket,
                                       ...
                                       s3_kms_extra_args=s3_kms_extra_args)
        LOG.info("[s3] Successfully upload ip done to \
    ...
    def _wait_for_ip_config(self):
        ...
        while time_elapsed < SAGEMAKER_WAIT_TIME:
            # if retry failed, s3_client list_objects will log and exit 500
            response = self._s3_client.list_objects(bucket=self._bucket,
