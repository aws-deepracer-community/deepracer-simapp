...
S3_ERROR_MSG_FORMAT = "S3 failed, retry after {0} seconds. \
                       Re-try count: {1}/{2}: [S3_Bucket: {3}, S3_Key: {4}]: {5}"
...
class S3Client():
    ...
    def list_objects(self, bucket, prefix):
        '''list object from s3 with retry logic
        ...
        try:
            return self._exp_backoff(action_method=self._get_s3_client().list_objects,
                                     Bucket=bucket,
                                     Prefix=prefix)
        except botocore.exceptions.ClientError:
