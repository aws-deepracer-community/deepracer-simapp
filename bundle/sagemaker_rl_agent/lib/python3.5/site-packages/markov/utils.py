...
def do_model_selection(s3_bucket, s3_prefix, region, checkpoint_type=BEST_CHECKPOINT, s3_endpoint_url=None):
    ...
    try:
        ...
        if model_checkpoint is None or not isinstance(model_checkpoint, str):
            logger.info("Exit model selection because model_checkpoint is {} of type {}".\
                format(model_checkpoint, type(model_checkpoint)))
            return False
