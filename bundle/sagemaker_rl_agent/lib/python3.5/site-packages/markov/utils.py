...
def get_best_checkpoint_num(s3_bucket, s3_prefix, region, s3_endpoint_url=None):
    ...
    """
    checkpoint_num = -1
    best_checkpoint_name = get_best_checkpoint(s3_bucket, s3_prefix, region, s3_endpoint_url)
    if best_checkpoint_name and len(best_checkpoint_name.split("_Step")) > 0:
        ...
        checkpoint_num = get_last_checkpoint_num(s3_bucket, s3_prefix, region, s3_endpoint_url)
    return checkpoint_num


def get_last_checkpoint_num(s3_bucket, s3_prefix, region, s3_endpoint_url=None):
    ...
    last_checkpoint_name = get_last_checkpoint(s3_bucket, s3_prefix, region, s3_endpoint_url)
...
def copy_best_frozen_model_to_sm_output_dir(s3_bucket, s3_prefix, region,
                                            source_dir, dest_dir, s3_endpoint_url=None):
...
def get_last_checkpoint(s3_bucket, s3_prefix, region, s3_endpoint_url=None):
    return get_deepracer_checkpoint(s3_bucket=s3_bucket,
                                    s3_prefix=s3_prefix,
                                    region=region,
...
def get_deepracer_checkpoint(s3_bucket, s3_prefix, region, checkpoint_type, s3_endpoint_url=None):
...
def do_model_selection(s3_bucket, s3_prefix, region, checkpoint_type=BEST_CHECKPOINT, s3_endpoint_url=None):
    ...
    try:
        ...
        s3_client = boto3.Session().client('s3', region_name=region, endpoint_url=s3_endpoint_url, config=get_boto_config())
...
def has_current_ckpnt_name(s3_bucket, s3_prefix, region, s3_endpoint_url=None):
...
def make_compatible(s3_bucket, s3_prefix, region, ready_file, s3_endpoint_url=None):
