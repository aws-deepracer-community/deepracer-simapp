...
def set_reward_and_metrics(reward_params, step_metrics, agent_name, pos_dict, track_data,
                           data_dict, action, json_actions, car_pose, start_ndist_offset=0.0):
    '''Populates the reward_params and step_metrics dictionaries with the common
       ...
       start_ndist_offset (float): start position offset from the finish lane
    '''
    try:
        ...
        # For training, current progress refers to the progress w.r.t to the starting position.
        # data_dict['start_ndist'] will be changed per episode to make sure agent can explore the
        # whole track and start_ndist_offset is always 0.0.
        # For evaluation, current progress refers to the progress w.r.t to the starting/finishing
        # line no matter whether the starting position is. data_dict['start_ndist'] and
        # start_ndist_offset will always be the same refer to its racing start position on the
        # track.
        current_progress = current_ndist - data_dict['start_ndist'] + start_ndist_offset
