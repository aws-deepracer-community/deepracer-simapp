...
from markov.utils import force_list, restart_simulation_job, cancel_simulation_job
    ...
    try:
        ...
        if os.path.exists(local_queue_pickle_path):
            ...
            for agent_idx, _ in enumerate(yaml_dict[MODEL_S3_BUCKET_YAML_KEY]):
                tournament_candidate_queue.append((yaml_dict[MODEL_S3_BUCKET_YAML_KEY][agent_idx],
        ...
        log_and_exit(
            "Tournament node failed: s3_bucket: {}, yaml_key: {}, {}".format(s3_bucket,
                                                                             yaml_key, e),
            SIMAPP_SIMULATION_WORKER_EXCEPTION,
            SIMAPP_EVENT_ERROR_CODE_500)
