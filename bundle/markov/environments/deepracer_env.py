...
---------------------------------
Provide a partial ``config`` dict with any :class:`~markov.agent_ctrl.constants.ConfigParams`
...
Full control
------------
Build an :class:`~markov.agents.agent.Agent` yourself and pass it directly.
...
from markov.sensors.constants import Input
from markov.constants import SIMAPP_VERSION_5
...
def _build_agent(
    ...
    '''Build a default :class:`~markov.agents.agent.Agent` from its component parts.
...
class DeepRacerEnv(gymnasium.Env):
    ...
    Args:
        reward_fn: Callable ``(params: dict) -> float``.  Required unless
            ...
            :class:`~markov.agent_ctrl.constants.RewardParam`.
        ...
        config: Partial override dict for the controller.  Keys are the
            string *values* of :class:`~markov.agent_ctrl.constants.ConfigParams`.
            Any key not present keeps its default value.
        agent: Fully-initialised :class:`~markov.agents.agent.Agent`.
    ...
    @property
    def agent(self):
        '''The underlying :class:`~markov.agents.agent.Agent` instance.'''
        return self._agent
