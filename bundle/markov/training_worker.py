...
from markov.log_handler.constants import (SIMAPP_EVENT_ERROR_CODE_500,
                                          ...
                                          SAGEONLY_TRAINING_JOB_PID_FILE_PATH)
from markov.constants import (SIMAPP_VERSION_2, TRAINING_WORKER_PROFILER_PATH, DEEPRACER_JOB_TYPE_ENV,
                              DeepRacerJobType)
...
def main():
    ...
    parser.add_argument('--s3_bucket',
                        ...
                        type=str,
                        default=os.environ.get("SAGEMAKER_SHARED_S3_BUCKET", "sagemaker-s3"))
...
if __name__ == '__main__':
    try:
        # Writing PID of the sagemaker instance. If there is a fault on SimApp code,
        # this PID is used to terminate the training process
        if os.environ.get(DEEPRACER_JOB_TYPE_ENV) == DeepRacerJobType.SAGEONLY.value:
            with open(SAGEONLY_TRAINING_JOB_PID_FILE_PATH, "w") as fp:
                fp.write("%s\n" % str(os.getpid()))
                logger.info("Writing training job pid to %s: %s", SAGEONLY_TRAINING_JOB_PID_FILE_PATH,
                            str(os.getpid()))
        main()
    except Exception as ex:
