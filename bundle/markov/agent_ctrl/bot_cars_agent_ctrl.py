...
from markov.agent_ctrl.utils import get_normalized_progress, get_relative_pos
...
class BotCarsCtrl(AgentCtrlInterface, AbstractTracker):
    ...
    def _get_dist_from_sim_time(self, initial_dist, sim_time):
        ...
        '''
        seconds_elapsed = max(sim_time - self.start_sim_time - self.bot_car_crash_count * \
                              self.penalty_seconds - self.pause_time_before_start, 0.0)
    ...
    def reset_agent(self):
        ...
        self.bot_car_crash_count = 0
        self.bot_car_phase = AgentPhase.PREPARE.value
        self.pause_duration = self.pause_time_before_start
    ...
    def update_agent(self, action):
        ...
        self._update_track_data_bot_car_poses()
        for bot_car_name, bot_car_pose in zip(self.bot_car_names, self.bot_car_poses):
            # get bot head point location
            origin = np.array([bot_car_pose.position.x,
                               bot_car_pose.position.y,
                               bot_car_pose.position.z])
            translation = RELATIVE_POSITION_OF_FRONT_OF_CAR
            rotation = np.array([bot_car_pose.orientation.x,
                                 bot_car_pose.orientation.y,
                                 bot_car_pose.orientation.z,
                                 bot_car_pose.orientation.w])
            bot_point = Point(get_relative_pos(origin, translation, rotation))
            bot_car_progress = self.track_data.get_norm_dist(bot_point) * 100.0
            ...
            for agent_name, agent_info in agents_info_map.items():
                ...
                if 'bot_car' in crashed_object_name:
                    ...
                    if racecar_progress > bot_car_progress:
                        ...
                        break
        elif self.bot_car_phase == AgentPhase.PAUSE.value or \
                self.bot_car_phase == AgentPhase.PREPARE.value:
