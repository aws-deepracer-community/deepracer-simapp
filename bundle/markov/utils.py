    ...
    if(os.environ.get("JOB_TYPE", "") == "training"):
        ...
        try: 
            ...
            for filename in filenames:
                # option to pass custom ros log filepath, otherwise just keep using the job name as always
                ros_log_file_path = os.environ.get("ROS_LOG_FILE_PATH", os.environ.get("JOB_NAME"))
                s3_client.upload_file(bucket=os.environ.get("S3_ROS_LOG_BUCKET"),
                                            s3_key=os.path.normpath(
                                                    os.path.join(ros_log_file_path,
                                                        filename.split("/")[-1])),
                                            local_path=filename,
