...
class YamlFile():
    ...
    @property
    def is_leaderboard_job(self):
        '''return True for leaderboard job, else False'''
        self.get_yaml_values()
        return self._is_leaderboard_job
    ...
    def get_yaml_values(self):
        ...
        if not self._yaml_values:
            ...
            # Update world name based on track direction (if passed).
            self._update_world_name()
            ...
            if self._agent_type == AgentType.VIRTUAL_EVENT.value:
                ...
                self._kinesis_webrtc_signaling_channel_name = force_list(self._yaml_values[
                    YamlKey.KINESIS_WEBRTC_SIGNALING_CHANNEL_NAME.value])
                self._is_multicar = len(self._kinesis_webrtc_signaling_channel_name) > 1
            else:
    ...
    def _update_world_name(self):
        '''Configure the world name to be updated based on track direction.
        '''
        world_name = str(self._yaml_values.get(YamlKey.WORLD_NAME.value))
        LOG.info("World name passed in YAML: {}".format(world_name))
        if YamlKey.TRACK_DIRECTION_CLOCKWISE.value in self._yaml_values:
            LOG.info("Track direction passed in YAML: {}".
                        format(self._yaml_values.get(YamlKey.TRACK_DIRECTION_CLOCKWISE.value)))
            is_clockwise = str2bool(self._yaml_values.get(YamlKey.TRACK_DIRECTION_CLOCKWISE.value))
            if is_clockwise:
                world_name = world_name + WorldNameSuffix.CLOCKWISE.value
            else:
                world_name = world_name + WorldNameSuffix.COUNTER_CLOCKWISE.value
            self._yaml_values[YamlKey.WORLD_NAME.value] = world_name
            LOG.info("Updating world name based on track direction YAML parameter: {}".
                        format(self._yaml_values.get(YamlKey.WORLD_NAME.value)))
            os.environ["WORLD_NAME"] = world_name
            # override local yaml file with updated WORLD_NAME
    ...
    def _postprocess_yaml_values(self):
        ...
        # set is_leaderboard_job as true if LEADERBOARD_NAME is not empty
        self._is_leaderboard_job = self._yaml_values.get(
            YamlKey.LEADERBOARD_NAME_YAML_KEY.value, "") != ""
