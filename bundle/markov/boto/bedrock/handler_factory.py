...
class HandlerFactory:
    ...
    def create_handler(model_id: str, 
        ...
        if "arn:aws:bedrock" in model_id_lower:
            ...
            # Strip cross-region inference profile prefix (e.g. eu., us., ap.)
            normalized_id = re.sub(r'^(us|eu|ap)\.', '', model_id_lower)
            # Direct model ID - identify by prefix pattern
            if normalized_id.startswith(("anthropic.", "claude")):
                handler = ClaudeHandler(model_id, region)
            elif normalized_id.startswith(("mistral.")):
                handler = MistralHandler(model_id, region)
            elif normalized_id.startswith(("meta.", "llama")):
                handler = LlamaHandler(model_id, region)
            elif normalized_id.startswith(("amazon.titan", "amazon.nova", "titan", "nova")):
                handler = NovaHandler(model_id, region)
            else:
