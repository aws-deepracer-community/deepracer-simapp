...
class LLMActionSpaceConfig(ActionSpaceConfigInterface):
    def __init__(self, action_space_type):
        """LLM training algorithm action space configuration

        Args:
            action_space_type (str): action space type used to identify what action space object to return
        """
        if action_space_type not in [ActionSpaceTypes.DISCRETE.value, ActionSpaceTypes.CONTINUOUS.value]:
            log_and_exit("Unsupported action space type passed while defining LLMActionSpaceConfig. \
                action_space_type: {}".format(action_space_type),
                         SIMAPP_SIMULATION_WORKER_EXCEPTION,
                         SIMAPP_EVENT_ERROR_CODE_500)
    ...
    def get_action_space(self, json_actions):
        ...
        try:
            if self.action_space_type == ActionSpaceTypes.CONTINUOUS.value:
                ...
                if json_actions[ModelMetadataKeys.STEERING_ANGLE.value][ModelMetadataKeys.LOW.value] \
                    >= json_actions[ModelMetadataKeys.STEERING_ANGLE.value][ModelMetadataKeys.HIGH.value] or \
                        json_actions[ModelMetadataKeys.SPEED.value][ModelMetadataKeys.LOW.value] \
                        >= json_actions[ModelMetadataKeys.SPEED.value][ModelMetadataKeys.HIGH.value]:
                    log_and_exit("Action space bounds are incorrect while defining LLMActionSpaceConfig. \
                        json_actions: {}".format(json_actions),
                                 SIMAPP_SIMULATION_WORKER_EXCEPTION,
                                 SIMAPP_EVENT_ERROR_CODE_500)
                original_low = np.array([json_actions[ModelMetadataKeys.STEERING_ANGLE.value]
            ...
            log_and_exit('Error while getting action space in LLMActionSpaceConfig: {}'.format(ex),
                         SIMAPP_SIMULATION_WORKER_EXCEPTION,
                         SIMAPP_EVENT_ERROR_CODE_500)
